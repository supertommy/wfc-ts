...
| H45 | speculative multi-observe batches with restart safety | 2 (valid+det; algorithm/search change) | speed (fewer propagate/flush cycles) + success risk | TODO | Post-H44 phase profile supports a script-local prototype: circuit still spends ~82% in propagation with 772 drains, rooms ~73% with 519 drains. Start conservative (batch size 2, maybe 4 only if promising), keep deterministic restarts, and gate VALID+DET + success rate + speed. High risk: stale choices may increase contradictions or regress full-run speed. |
