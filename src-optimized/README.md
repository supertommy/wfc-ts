...
| H10 | preliminary-action pruning: precompute per-cell reachable patterns (adjacency-reachability fixpoint from boundary), pre-ban the rest before run | 2 (valid+det) | ALL THREE axes | TODO | IFR-aligned (propagation done once, in advance); shrinks wave + cuts propagation + raises success. Do first. |
