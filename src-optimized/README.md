...
| H42 | FIFO propagation queue discipline in optimized model | 2 (valid+det; ordering changes) | speed (prop wall) | TODO | Promote H40's signal minimally: process pending bans in insertion order while appending derived bans at the tail; keep direction order 0,1,2,3 and existing `ban()` semantics/dirty-list behavior. Gate full solver and revert if speed regresses. |
