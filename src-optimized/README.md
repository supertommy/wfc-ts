...
| H41 | static default-MRV specialized propagation path (no eval) | 1/2 (implementation specialization; preserves algorithm under MRV) | speed (prop wall) | TODO | Promote the H39 learning cleanly: in `src-optimized/model.ts`, use a hand-written MRV/default propagate path that inlines ban side effects and unrolls directions, while keeping the existing generic path for Entropy/Scanline or as fallback. Gate full solver: typecheck, prove-harness, measure-speedup knots/circuit/rooms. |
