...
| H39 | generated/specialized hot propagation kernel per model shape | 1/2 (implementation specialization; should preserve algorithm) | speed (prop + observe overhead) | PROTOTYPE-PASSED | Script-local `scripts/cpu-generated-propagate-proto.ts` matched wave+sums (`wDiff=0`, `sDiff=0`) and showed drain-only wins on prop-bound targets: circuit `0.0395→0.0295ms`, rooms `0.0125→0.0092ms`, circuit-128 `0.0381→0.0294ms`; knots tiny cascade regressed `0.0010→0.0013ms`. Do not ship `new Function`/eval; next candidate is H41 static MRV-specialized propagation in `model.ts` with generic fallback. |
| H41 | static default-MRV specialized propagation path (no eval) | 1/2 (implementation specialization; preserves algorithm under MRV) | speed (prop wall) | REVERTED | Hand-written MRV-specialized `model.ts` path passed typecheck/prove-harness but regressed full-run speed: circuit fell to ~`1.8x` vs reference and rooms to ~`2.2x`, below Round-3 baseline. H39's eval-generated drain win did not transfer to a large hand-written method; likely JIT/register/deopt costs. Reverted model change. |
| H40 | propagation work ordering experiments (FIFO/spatial/ring vs LIFO) | 2 (valid+det; ordering changes) | speed (cache locality) | PROTOTYPE-PASSED | Script-local `scripts/cpu-propagation-order-proto.ts` matched wave+sums for all variants. FIFO variants showed consistent drain-only wins: circuit ~`1.11x`, rooms ~`1.17x`, circuit-128 ~`1.12x`; direction order had little effect. Next candidate H42 promotes the simplest `fifo-0123` queue discipline with full gates. |
| H42 | FIFO propagation queue discipline in optimized model | 2 (valid+det; ordering changes) | speed (prop wall) | REVERTED | Minimal FIFO `propagate()` passed typecheck/prove-harness but regressed full-run speed: circuit ~`2.35-2.53x`, rooms ~`2.75-2.87x` vs Round-3 baseline ~`2.77x/3.22x`. Reverted. H40's drain-only signal did not survive full solver measurement. |
| H43 | precomputed compatible offsets for propagation inner loop | 1 (byte-id; layout/precompute) | speed (prop inner decrement wall) | KEPT | Added `propCompatOffset[start+l]=t2*4+d` parallel to `propData` and used it for the common-case decrement; read `propData` only on zero. Gates PASS. Higher-rep A/B: circuit `2.539→2.437ms` (~4% win), knots `1.035→1.024ms` flat/slight win, rooms noise-flat (`1.117→1.151ms` in high-rep, earlier runs flat/opposite). Tiny memory cost; keep for circuit wall. |
| H44 | precomputed neighbor compatible bases | 1 (byte-id; layout/precompute) | speed (prop outer/base setup) | KEPT | Added `neighborCompatBase[i*4+d]=neighbor*T4|-1`, used it in propagation common path, and read `neighbors[nidx]` only on zero branch. Gates PASS. Higher-rep A/B vs H43: knots `1.016→0.984ms`, circuit `2.376→2.293ms`, rooms `1.086→1.068ms`. Memory +`count*4*4`; speed win accepted. |
| H45 | speculative multi-observe batches with restart safety | 2 (valid+det; algorithm/search change) | speed (fewer propagate/flush cycles) + success risk | REJECTED | Script-local prototype tested naive batch-2 plus spatially-separated batch-2/4. Naive batching exhausted restart budget on all targets. Spatial separation kept knots valid but was slower (`0.313x` for b2sep) and still exhausted circuit/rooms. Stale unpropagated choices create contradictions/overhead faster than saved drains repay. |


## Round 4 CPU conclusion

Round 4 is stopped. The safe precompute wins H43/H44 are kept; the algorithm/search-shape paths H37/H38/H40/H42/H45 were rejected or reverted after measurement. Current optimized JS remains the shippable path, already ahead of comparable external implementations. Next work should move to Phase 4c OSS polish (README, visualizer, learning guide, external benchmark refresh, packaging), not another CPU ratchet loop unless a genuinely new algorithmic seam appears.
...
Candidate order: H37 dirty-cell bitset propagation, H38 cell-batched AC-4, H39 generated/specialized kernels, H40 propagation ordering. H37/H38/H41/H42 failed promotion; H39/H40 produced prototype-only signals. Iteration 7 STALL→IDEATE found new candidates H43-H45. Continue with H43 first because it directly attacks the common-case propagation decrement wall while preserving AC-4 semantics.
