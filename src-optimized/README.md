...
| H43 | precomputed compatible offsets for propagation inner loop | 1 (byte-id; layout/precompute) | speed (prop inner decrement wall) | TODO | TRIZ Preliminary Action / Taking Out from Round-4 stall ideation. Build `propCompatOffset` parallel to `propData`, storing `t2*4+d` (Uint8 for current T4<256). Hot loop becomes `cidx = base2 + propCompatOffset[start+l]`; read `propData[start+l]` only on zero to call `ban`. Removes `t2*4+d` multiply/add and common-case tile-id read from every decrement while preserving AC-4 semantics. High-payoff because it targets the inner decrement wall directly. |
| H44 | precomputed neighbor compatible bases | 1 (byte-id; layout/precompute) | speed (prop outer/base setup) | TODO | TRIZ Preliminary Action. Store `neighborCompatBase[i*4+d] = neighbor*T4` or `-1` to remove `i2*T4` per popped ban per direction. Lower payoff than H43 because it hits the outer direction setup, not each inner decrement; test only if H43 keeps or shows nearby signal. |
| H45 | speculative multi-observe batches with restart safety | 2 (valid+det; algorithm/search change) | speed (fewer propagate/flush cycles) + success risk | TODO | TRIZ Skipping / Periodic Action. Collapse a small batch of spatially separated MRV cells before propagation, then drain once; rely on deterministic restart budget for contradictions. Potentially high payoff by reducing observe→propagate cycles, but high risk to success and output/order. Prototype script-local/full-run only after low-risk H43/H44. |
