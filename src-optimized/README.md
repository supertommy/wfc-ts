...
| H37 | dirty-cell + bitset support propagation (AC-3-ish domain filtering) | 2 (valid+det; algorithm rewrite) | speed (propagation wall) | TODO | Round 4 reopened by user. Prototype script-local first: `waveMask[cell,lane]`, support masks by `(d,t,lane)`, queue changed cells, recompute neighbor support with bitwise intersections. Goal: coalesce many tile bans per cell and replace AC-4 decrement writes with bit tests. Risk: scans live tiles and may overwork tiny frontiers. Promote only if prototype beats current optimized on real gates. |
| H38 | cell-batched AC-4 propagation | 2 (valid+det; algorithm rewrite) | speed (propagation wall) | TODO | Keep support counts but batch changed tiles by cell/direction so a cell that loses many tiles is propagated once/coherently instead of many independent `(cell,tile)` stack pops. Lower risk than H37, possibly less payoff. |
